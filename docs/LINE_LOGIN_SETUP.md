...
## 8) กล่องบัญชี + ออกจากระบบทุกที่ — 24 กันยายน 2569

- `src/components/AccountCard.jsx`: หน้าตั้งค่า (`/settings`) แสดงทั้งสองสถานะ (ยังไม่ login = ปุ่ม LINE + บัญชี JIA —
  ลิงก์ "ล็อกอิน LINE ใน ตั้งค่า" จากหน้าเข้าคลาสมาจบที่นี่), หน้าบทเรียน (`/learn`) และหน้าใบประกาศแสดงเฉพาะตอน login แล้ว
  (ชื่อ + ช่องทาง LINE/บัญชี JIA + ปุ่ม "ออกจากระบบ") — ด่าน login หลังบทแรก (`LineLoginGate`) ไม่เปลี่ยน
- "ออกจากระบบ" (`src/utils/accountLogout.js`): ส่งความก้าวหน้าที่ค้างขึ้นบัญชี (รอไม่เกิน 4 วิ) → `supabase.auth.signOut({scope:'local'})`
  (เครื่องอื่นของผู้เรียนไม่หลุด) → ล้างผู้เรียนในเครื่อง (`firstaid.learner`) ให้คนถัดไปได้ผู้เรียนนิรนามคนใหม่ →
  ไป `https://class.jiacpr.com/sso/logout?client_id=firstaid&redirect_uri=<origin>/auth/hub/callback&return_path=<หน้าเดิม>`
  ซึ่งปิด session ของ Hub แล้วพากลับมา (jia-learning-hub `docs/unified-identity.md` §9) — กด "เข้าสู่ระบบด้วยบัญชี JIA" ครั้งต่อไป
  ต้อง login ใหม่ ไม่ถูกพาเข้าบัญชีเดิม
- ต้องล้างผู้เรียนในเครื่องด้วย: ถ้าเหลือ learner id ของคนก่อน คนถัดไปที่ login บัญชีตัวเองจะถูก `linkLearnerToAuth` rekey
  ความก้าวหน้าของคนก่อนเข้าบัญชีตัวเอง; ข้อมูลใน Dexie ใต้ id เดิมยังอยู่ และความก้าวหน้าของบัญชีกลับมาครบเมื่อ login ใหม่ (`pullSync`)
- LINE เองจำ session ไว้ที่ access.line.me — ครั้งถัดไป LINE จะถามว่า "ดำเนินการต่อในชื่อ …" และมีปุ่มเปลี่ยนบัญชีให้ (ควบคุมจากเว็บนี้ไม่ได้)
